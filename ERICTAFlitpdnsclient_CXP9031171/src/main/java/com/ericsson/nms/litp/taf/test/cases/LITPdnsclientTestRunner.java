...
public class LITPdnsclientTestRunner extends TorTestCaseHelper {
    
    Logger logger = Logger.getLogger(LITPdnsclientTestRunner.class);
    ...
    @TestId(id = "CXP9031171-1", title = "Upgrade ERIClitpdnsclient rpm on the MS")
    @Test(groups={"ACCEPTANCE"})
    public void upgradeERIClitpdnsclientRPM() throws TimeoutException, FileNotFoundException {

        rpmUpgradeOperator.initialise("dnsclient");
     ...
     * @PRIORITY HIGH
     */
    @TestId(id = "CXP9031171-2", title = "Run python test cases for ERIClitpdnsclient package")
    @Test(groups={"CDB_REGRESSION", "ACCEPTANCE"})
    public void runERIClitpdnsclientTests() {
     ...
     * @PRE Execution of test {@link #runERIClitpdnsclientTests()}
     * @PRIORITY HIGH
     * @param className
     * @param name
     * @param failures
     */
    @TestId(id = "CXP9031171-3", title = "Parse xml outputs from python tests")
